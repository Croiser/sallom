...
// Asaas Payments
router.post('/asaas/pix', authenticateToken, async (req, res) => {
    try {
        const { planId, billingCycle, amount, customerInfo } = req.body;
        const uid = req.user?.id;
        const email = req.user?.email;
        const customerId = await asaas.getOrCreateCustomer(customerInfo.name, customerInfo.cpfCnpj, email, customerInfo.phone);
...
router.post('/asaas/card', authenticateToken, async (req, res) => {
    try {
...
// WhatsApp Settings
router.get('/whatsapp-settings', authenticateToken, async (req, res) => {
    try {
        const uid = req.user?.id;
...
router.post('/whatsapp-messages', authenticateToken, async (req, res) => {
    try {
...
router.post('/wallet/toggle', authenticateToken, async (req, res) => {
    try {
        const ownerUid = req.user?.id;
...
router.post('/clients/:id/redeem', authenticateToken, async (req, res) => {
    const ownerUid = req.user?.id;
...
router.post('/products', authenticateToken, async (req, res) => {
    ...
    const product = await prisma.product.create({
        data: {
            name, price, cost, stock, category, ownerUid: req.user?.id
        }
    });
...
router.get('/superadmin/plans', authenticateToken, isSuperAdmin, async (req, res) => {
...
router.put('/superadmin/plans/:id', authenticateToken, isSuperAdmin, async (req, res) => {
...
router.get('/admin/users/:id/usage', authenticateToken, isSuperAdmin, async (req, res) => {
    try {
        const userId = req.params.id;
...
router.put('/admin/users/:id', authenticateToken, isSuperAdmin, async (req, res) => {
    try {
...
router.put('/admin/users/:id/status', authenticateToken, isSuperAdmin, async (req, res) => {
    const { status } = req.body;
    await prisma.user.update({
        where: { id: req.params.id },
        data: { status }
...
// SuperAdmin Routes
router.get('/superadmin/stats', authenticateToken, async (req, res) => {
...
router.get('/superadmin/tenants', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
...
router.get('/superadmin/plans', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
        const plans = await prisma.plan.findMany();
...
router.get('/superadmin/tenant-usage/:userId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
...
router.post('/superadmin/tenants/:userId/wallet/recharge', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
        const { userId } = req.params;
...
router.put('/superadmin/tenants/:userId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
        const { userId } = req.params;
...
router.put('/superadmin/plans/:planId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
...
router.post('/superadmin/plans', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
...
router.delete('/superadmin/plans/:planId', authenticateToken, async (req, res) => {
    if (req.user.role !== 'admin')
        return res.status(403).json({ error: 'Forbidden' });
    try {
        const { planId } = req.params;
...
// --- ANAMNESIS ROUTES ---
router.get('/clients/:id/anamnesis', authenticateToken, async (req, res) => {
    try {
...
router.post('/clients/:id/anamnesis', authenticateToken, async (req, res) => {
    try {
...
router.delete('/anamnesis/:id', authenticateToken, async (req, res) => {
    try {
