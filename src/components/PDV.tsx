...
export default function PDV() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-[calc(100vh-140px)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredItems.map(item => (
              <motion.button
                ...
                className="bg-white p-5 rounded-3xl border border-zinc-200 shadow-sm hover:shadow-premium hover:border-brand-500/50 transition-all text-left flex flex-col h-full group"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className={`p-2.5 rounded-2xl ${item.type === 'product' ? 'bg-amber-50 text-amber-600' : 'bg-brand-50 text-brand-500'}`}>
                    {item.type === 'product' ? <Package size={20} /> : <Sparkles size={20} />}
                  </div>
                  <span className="text-xl font-display font-black text-brand-500">
                    R$ {item.price.toFixed(2)}
                  </span>
                </div>
                <h4 className="font-bold text-zinc-900 leading-tight mb-2 group-hover:text-brand-500 transition-colors">{item.name}</h4>
                {item.type === 'product' && (
                  <div className="mt-auto flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-zinc-400">
      ...
      <div className="w-full lg:w-[420px] flex flex-col bg-white rounded-[2.5rem] border border-zinc-200 shadow-premium overflow-hidden sticky top-0">
        <div className="p-8 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
          <h3 className="text-xl font-display font-black flex items-center gap-3 text-zinc-900">
            <ShoppingCart className="text-brand-500" size={24} />
            Carrinho
          </h3>
          <span className="bg-brand-500 text-white text-xs font-black px-2.5 py-1 rounded-full">{cart.length}</span>
        </div>
            ...
            {cart.length === 0 ? (
              ...
              cart.map(item => (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100 group"
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="min-w-0 flex-1 pr-4">
                      <p className="font-bold text-sm text-zinc-900 truncate">{item.name}</p>
                      <p className="text-xs text-brand-500 font-bold">R$ {item.price.toFixed(2)}</p>
                    </div>
                    <button 
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center bg-white rounded-xl border border-zinc-200 p-1">
                      <button 
                        ...
                        className="p-1 hover:bg-zinc-50 rounded-lg text-zinc-500 transition-colors"
                      >
                        <Minus size={14} />
                      </button>
                      <span className="px-3 text-sm font-black text-zinc-900 min-w-[32px] text-center">{item.quantity}</span>
                      <button 
                        onClick={() => updateQuantity(item.id, 1)}
                        className="p-1 hover:bg-zinc-50 rounded-lg text-zinc-500 transition-colors"
                      >
        ...
        <div className="p-8 bg-zinc-50/80 border-t border-zinc-100 space-y-6">
          <div className="grid grid-cols-1 gap-4">
                ...
                >
                  <div className="bg-rose-50 border border-rose-100 rounded-2xl p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[10px] font-black uppercase text-rose-500 tracking-wider">Débito Pendente</p>
                        <p className="text-sm font-bold text-rose-900">R$ {(clients.find(c => c.id === selectedClient)?.pendingDebt || 0).toFixed(2)}</p>
                      </div>
                    </div>
                    <button
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="input-premium py-2.5 text-sm font-bold text-brand-600"
              >
          ...
          <div className="flex gap-2">
            {[
              ...
              >
                <method.icon size={16} />
                <span className="text-[10px] font-black uppercase text-center">{method.label}</span>
              </button>
            ))}
          </div>
          ...
          <div className="pt-2">
            <div className="flex justify-between items-baseline mb-6">
              <span className="text-zinc-500 font-bold">Total da Venda</span>
              <div className="text-right">
                <p className="text-3xl font-display font-black text-zinc-900 tracking-tighter">
                  <span className="text-sm text-brand-500 mr-1.5 uppercase font-sans">R$</span>
                  {total.toFixed(2)}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className={`
                w-full h-16 rounded-[1.25rem] font-display font-black text-lg uppercase tracking-tight flex items-center justify-center gap-3 transition-all
                ${success 
