...
export const asaas = {
  async getOrCreateCustomer(name: string, cpfCnpj: string, email: string, phone: string) {
  ...
  async createPixPayment(customerId: string, amount: number, externalReference: string) {
  ...
  async createCardPayment(customerId: string, amount: number, externalReference: string, card: any, creditCardHolderInfo: any) {
    const payment = await asaasFetch('/payments', {
      method: 'POST',
      body: JSON.stringify({
        customer: customerId,
