  ...
  cron.schedule('*/30 * * * *', async () => {
    try {
      ...
      for (const app of upcomingAppointments) {
        if (sentReminders.has(app.id)) continue;
        if (!app.phone) continue;

        const uid = app.ownerUid;
        const settings = await (prisma as any).whatsappSettings.findUnique({ where: { uid } });
      ...
      sentReminders.forEach(async (id) => {
        const app = await (prisma as any).appointment.findUnique({ where: { id }, select: { date: true } });
